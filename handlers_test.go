...
func BenchmarkSprintf(b *testing.B){
	...
	rr1 := newRequestRecorder(req1, "GET", "/books/:isdn", BookShow)
	if rr1.Code != 404 {
		b.Error("Expected response code to be 404")
	}
	// expected response
	er1 := "{\"error\":{\"status\":404,\"title\":\"Record Not Found\"}}\n"
	if rr1.Body.String() != er1 {
		b.Error("Response body does not match")
	}

	b.Log("When the book exists")
	// Create an entry of the book to the bookstore map
	testBook := &Book{
		ISDN:   "111",
		Title:  "test title",
	...
	rr2 := newRequestRecorder(req2, "GET", "/books/:isdn", BookShow)
	if rr2.Code != 200 {
		b.Error("Expected response code to be 200")
	}
	// expected response
	er2 := "{\"meta\":null,\"data\":{\"isdn\":\"111\",\"title\":\"test title\",\"author\":\"test author\",\"pages\":42}}\n"
	if rr2.Body.String() != er2 {
	...
	//bookstore["111"] = testBook
	// A request with an existing isdn
	req2, err2 := http.NewRequest("GET", "/books", nil)
	if err2 != nil {
		b.Fatal(err)
	}
	rr2 = newRequestRecorder(req2, "GET", "/books", BookIndex)
	if rr2.Code != 200 {
		b.Error("Expected response code to be 200")
	}
	// expected response
